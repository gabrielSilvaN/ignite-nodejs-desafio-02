...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const conditionNotPro = user.pro === false && user.todos.length < 10;

  if (conditionNotPro || user.pro) {
    return next();
  }
  ...
  if (user.pro === false && user.todos.length >= 10) {
    return response
      .status(403)
      .json({ error: "Cannot create more than 10 todos in basic plan" });
  }
}
